...
<body>
	...
	<table border="1">
		...
		<c:forEach items="${list }" var="score">					<!-- foreach -->
			<tr>
				<td>
					<!--  eq: == / ne: != / empty: null -->
					<c:if test="${score.name eq '이름10' }">			<!-- if -->
						<c:out value="홍길동"></c:out>
					</c:if>
					<c:choose>										<!-- switch -->
						<c:when test="${score.name eq '이름20' }">	<!-- case: 이름20 -->
							<c:out value="${score.name }님!"></c:out>
						</c:when>
						<c:when test="${score.name eq '이름30' }">	<!-- case: 이름30 -->
	...
	<c:forEach var="i" begin="1" end="1" step="2">
		${i }<br>
	</c:forEach>
